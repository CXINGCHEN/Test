...
public class EKFAlgorithm {
    ...
    private int dimenX = 2; // for 2D position (latitude, longitude)


    // kinematics description
    private DenseMatrix64F F,H,Q,R;
    private int dimenZ = 2; // Two measurements (lat, long) from each of the three methods: GNSS, wifi positioning, PDR estimation
    ...
    public EKFAlgorithm(double [] x, double [][] P){
        this.x = new DenseMatrix64F(x.length, 1, false, x);
        this.P = new DenseMatrix64F(P);
    }
    public EKFAlgorithm(DenseMatrix64F x, DenseMatrix64F P){
        this.x = x;
        this.P = P;
    }
     ...
     * @param algorithmInputDataList List of EKFInputData objects with the 3 LatLong
    ...
    public EKFAlgorithm applyEKFAlgorithm ( List<EKFInputData> algorithmInputDataList,
                                                EKFAlgorithm initialEstimates){
        ...
        return new EKFAlgorithm(x, P);
    }
    private void creationAPrioriEstimates(EKFAlgorithm initialEstimates) {
        // Initial mean x
    ...
    private void creationJacobianMatrices(List<EKFInputData> algorithmInputDataList) {
        /* F, Jacobian of f */
        ...
        for (i = 0; i < dimenZ; i++){
            temp = Math.pow((x.get(0) - algorithmInputDataList.get(i).LatLong[0]),2) +
                    Math.pow((x.get(1) - algorithmInputDataList.get(i).LatLong[1]),2);
            computed_dist[i] = Math.sqrt(temp);
        }
        // Filling matrix H
        double hijx, hijy;
        for (i = 0; i < dimenZ; i++){
            hijx = (x.get(0) - algorithmInputDataList.get(i).LatLong[0])/computed_dist[i];
            hijy = (x.get(1) - algorithmInputDataList.get(i).LatLong[1])/computed_dist[i];
            H.set(i, 0, hijx);
            H.set(i, 1, hijy);
        }
    }
    ...
    private void creationNoiseCovarianceMatrices() {
        ...
        double[] sigma_meas = new double[]{0.1, 0.1};
